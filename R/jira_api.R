...
#' NULL
get_boards <- function() {
  all_boards <- jira_api("/rest/agile/1.0/board") %>%
    purrr::pluck("content", "values")
  dplyr::bind_cols(dplyr::select(all_boards, -.data$location),
                   purrr::pluck(all_boards, "location"))
}
